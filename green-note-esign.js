/* Shared Green Note signature UI for Institute Head and Staff. Aadhaar input is checked locally only. */
(()=>{'use strict';
...
const time=s=>new Intl.DateTimeFormat('en-IN',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:true,timeZone:'Asia/Kolkata'}).format(new Date(s))+' IST';
function seal(n){if(!n?.note_esign_at)return '<span class="gn-esign-pending">Awaiting email verified signature</span>';return `<div class="hr-esign-seal gn-note-seal" role="img" aria-label="Portal e-signature for ${esc(n.note_esign_name)}, ${esc(n.note_esign_designation)}, ${esc(time(n.note_esign_at))}"><div class="hr-seal-label"><svg viewBox="0 0 18 18" aria-hidden="true"><rect x="1" y="1" width="16" height="16" rx="2"/><path d="m4 9 3 3 7-7"/></svg><span>Digitally Signed</span></div><div class="hr-seal-body"><svg class="hr-seal-watermark" viewBox="0 0 300 100" preserveAspectRatio="none" aria-hidden="true"><path d="M 22 36 L 76 84 L 278 10"/></svg><strong>${esc(n.note_esign_name)}</strong><span>${esc(n.note_esign_designation)}</span><time>${esc(time(n.note_esign_at))}</time></div></div>`}

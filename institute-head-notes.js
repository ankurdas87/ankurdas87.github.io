...
async function openDelivery(id,mode){
 ...
 modalBody.innerHTML='<div class="ih-modal-note-no">'+esc(n?.note_number||"GREEN NOTE")+'</div><h3>'+esc(n?.subject||"Official Green Note")+'</h3><div class="ih-modal-parties"><div><small>FROM</small><strong>'+esc(mode==="inbox"?profileLine(sender):"Principal-cum-Secretary · BLC@Principal")+'</strong></div><div><small>TO</small><strong>'+esc(mode==="sent"?profileLine(recipient):"Principal-cum-Secretary · BLC@Principal")+'</strong></div><div><small>CATEGORY</small><strong>'+esc(labels[n?.category]||n?.category||"—")+'</strong></div><div><small>'+(mode==="inbox"?"RECEIVED":"SENT")+'</small><strong>'+new Date(r.sent_at).toLocaleString()+'</strong></div></div><div class="ih-modal-content">'+(n?.note_content||"")+'</div>'+(r.message?'<div class="ih-modal-remark"><b>Forwarding Remark</b><br>'+esc(r.message)+'</div>':"")+(mode==="inbox"?'<div class="ih-forward-box" id="ihForwardBox" hidden><div class="ih-dispatch-field"><label>TO STAFF MEMBER</label><select id="ihForwardRecipient"></select></div><div class="ih-dispatch-field"><label>FORWARDING REMARK</label><input id="ihForwardMessage" maxlength="1000" placeholder="Optional official remark"></div><button type="button" class="ih-forward-submit" id="ihForwardSubmit">Forward →</button></div>':"");
 modal.hidden=false
}

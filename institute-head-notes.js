...
function freshGreen(){greenViewingId=null;gWs.classList.remove("ih-green-readonly");if(gEsign)gEsign.hidden=false;const sealBox=gWs.querySelector(".ih-green-esign div");if(sealBox)sealBox.innerHTML="<small>AADHAAR-BASED E-SIGN</small><strong>Principal-cum-Secretary</strong><p>Save this Green Note, then verify your email code to sign.</p>";if(gWs)delete gWs.dataset.viewingId;if(gCat){gCat.disabled=false;gCat.value=""}if(gSub){gSub.readOnly=false;gSub.value=""}if(gEd){gEd.contentEditable="true";gEd.innerHTML=""}document.querySelectorAll("[data-green-cmd]").forEach(b=>b.disabled=false);if(gSave){gSave.hidden=false;gSave.disabled=false;gSave.textContent="Save Green Note"}if(gEsign)gEsign.disabled=!greenViewingId;const lab=document.querySelector('label[for="ihGreenFiles"]');if(lab)lab.hidden=false;clearFiles(gFiles,gFileList);if(gHead)gHead.textContent="OFFICIAL · PENDING ISSUE";if(gHint)gHint.textContent="Reserved preview · sequence is finalized with the official Green Note";if(gState)gState.innerHTML="<b></b>Official note not yet issued";previewGreen()}
...
function openGreen(id){
 ...
 gWs.dataset.viewingId=d.id;gCat.value=d.category||"";gSub.value=d.subject||"";gEd.innerHTML=d.body||"";gNum.textContent=d.noteNo||"";if(gHead)gHead.textContent="OFFICIAL · LOCKED";if(gHint)gHint.textContent="Locked official record · read only";if(gState)gState.innerHTML="<b></b>Read-only Green Note · sign before sending";lockGreen();showPrincipalSeal(d)
}

async function showPrincipalSeal(d){
 const box=gWs.querySelector('.ih-green-esign div');if(!box||!d.dbId)return;
 try{const note=await window.BLCGreenNoteESign.fetchNote(d.dbId);if(greenViewingId!==d.id)return;box.innerHTML='<small>AADHAAR-BASED E-SIGN</small>'+window.BLCGreenNoteESign.seal(note);gEsign.hidden=!!note.note_esign_at;if(note.note_esign_at)updateLocalGreen(d.id,{signedAt:note.note_esign_at});}
...
async function submitSend(e){
 ...
 try{const noteId=await ensureDbGreen(n);const signed=await window.BLCGreenNoteESign.fetchNote(noteId);if(!signed.note_esign_at)throw new Error("Verify and sign the Green Note before sending.");const ins=await c.from("staff_note_deliveries").insert({note_id:noteId,sender_id:u.id,recipient_id:recipient,message:message||null,status:"sent"});if(ins.error)throw new Error(ins.error.message);sendStatus("Note sent successfully.","good");statusToast("Note sent successfully","Opening Sent Notes…");$("#ihSendMessage").value="";attachSend(null);setTimeout(()=>setSection("sent"),800)}catch(err){sendStatus("Note could not be sent: "+(err.message||"Unknown error"),"bad")}finally{btn.disabled=false}
}
...
ySave?.addEventListener("click",saveYellow);gSave?.addEventListener("click",saveGreen);yConvert?.addEventListener("click",convertYellow);yCancel?.addEventListener("click",()=>setSection("all"));gCancel?.addEventListener("click",()=>setSection("all"));gEsign?.addEventListener("click",async()=>{const n=read(GKEY).find(x=>x.id===greenViewingId);if(!n)return;try{const id=await ensureDbGreen(n);window.BLCGreenNoteESign.open(id,()=>showPrincipalSeal({...n,dbId:id}))}catch(e){if(gState)gState.textContent=e.message}});

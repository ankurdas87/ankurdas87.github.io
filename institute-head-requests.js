(() => {
 ...
 function eSignSeal(person, when, extra='') {
  ...
  return `<div class="hr-esign-seal ${extra}" role="img" aria-label="E-signature for ${esc(who)}">
   <div class="hr-seal-top">BARPETA LAW COLLEGE</div>
   <div class="hr-seal-inner"><span class="hr-seal-tick" aria-hidden="true">✓</span><small>Digitally Signed by:</small><strong>${esc(who)}</strong><span>${esc(designation)}</span><em>${esc(signatureDate(when))}</em></div>
   <div class="hr-seal-bottom">BLC PORTAL E-SIGNATURE</div>
  </div>`;
 }

(() => {
 ...
 async function attachment() {
  ...
  if(a?.application_mode==='written'){
   ...
   const staffSignature=a.staff_esign_at?`<div class="hr-staff-digital"><span>✓ &nbsp; Digitally Signed</span><strong>${esc(staffFull)}</strong><small>${esc(staffRole)}</small><time>${esc(signatureDate(a.staff_esign_at))} IST</time></div>`:`<div class="hr-legacy-staff-sign">${esc(staffFull)}</div>`;
